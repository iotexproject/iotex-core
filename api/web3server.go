...
func (svr *web3Handler) getTransactionByBlockHashAndIndex(in *gjson.Result) (interface{}, error) {
	...
	blk, receipts, err := svr.coreService.BlockByHash(blkHash)
	...
	return svr.getTransactionFromActionInfo(blkHash, blk.Actions[idx], receipts[idx])
}
...
func (svr *web3Handler) getTransactionByBlockNumberAndIndex(in *gjson.Result) (interface{}, error) {
	...
	blk, receipts, err := svr.coreService.BlockByHash(blkMetas[0].Hash)
	if errors.Cause(err) == ErrNotFound || idx >= uint64(len(receipts)) || len(receipts) == 0 {
		return nil, nil
	}
	if err != nil {
	...
	return svr.getTransactionFromActionInfo(blkMetas[0].Hash, blk.Actions[idx], receipts[idx])
}
