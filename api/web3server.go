...
func (svr *web3Handler) estimateGas(in *gjson.Result) (interface{}, error) {
	from, to, gasLimit, _, value, data, height, err := parseCallObject(in)
	if err != nil {
		return nil, err
	}
	...
	case *action.Execution:
		if height == rpc.LatestBlockNumber {
			...
			estimatedGas, retval, err = svr.coreService.WithHeight(uint64(height.Int64())).EstimateExecutionGasConsumption(context.Background(), elp, from)
		}
