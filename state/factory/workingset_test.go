...
func TestWriteReadView(t *testing.T) {
	...
	_, v, err := ws.ReadView("test")
	require.Equal(protocol.ErrNoName, errors.Cause(err))
	require.Equal(v, nil)
	require.Equal(protocol.ErrNoName, errors.Cause(ws.Unload("", "", v)))
	require.Equal(v, nil)
	...
	acc := &state.Account{
		...
		Root:         hash.Hash256b([]byte("test")),
		CodeHash:     []byte("test"),
		VotingWeight: big.NewInt(0),
	}
	require.NoError(ws.Load("test", "test", acc))
	acc1 := &state.Account{}
	require.NoError(ws.Unload("test", "test", acc1))
	require.Equal(acc, acc1)
...
func TestValidateBlock(t *testing.T) {
	...
	t.Run("normal call", func(t *testing.T) {
		...
		require.NoError(sf.Validate(ctx, blk))
		require.NoError(sdb.Validate(ctx, blk))
	})

	t.Run("nonce error", func(t *testing.T) {
		blk := makeBlock(t, 3, hash.ZeroHash256, digestHash)
		require.Equal(action.ErrNonce, errors.Cause(sf.Validate(ctx, blk)))
		require.Equal(action.ErrNonce, errors.Cause(sdb.Validate(ctx, blk)))
	})

	t.Run("root hash error", func(t *testing.T) {
		blk := makeBlock(t, 1, hash.Hash256b([]byte("test")), digestHash)
		require.Equal(block.ErrReceiptRootMismatch, errors.Cause(sf.Validate(ctx, blk)))
		require.Equal(block.ErrReceiptRootMismatch, errors.Cause(sdb.Validate(ctx, blk)))
	})

	t.Run("delta state digest error", func(t *testing.T) {
		blk := makeBlock(t, 1, hash.ZeroHash256, hash.Hash256b([]byte("test")))
		require.Equal(block.ErrDeltaStateMismatch, errors.Cause(sf.Validate(ctx, blk)))
		require.Equal(block.ErrDeltaStateMismatch, errors.Cause(sdb.Validate(ctx, blk)))
	})
}
