	...
	for _, preEaster := range []bool{false, true} {
		for _, versioned := range []bool{true, false} {
			for _, ns := range []string{mns, "test1", "test can pass with any string here"} {
				var rtf daoRetrofitter
				if versioned {
					rtf = newDaoRetrofitterArchive(nil, ns)
				} else {
					rtf = newDaoRetrofitter(nil)
				}
				flusher, err := db.NewKVStoreFlusher(db.NewMemKVStore(), batch.NewCachedBatch(), rtf.flusherOptions(preEaster)...)
				r.NoError(err)
				stores = append(stores, newStateDBWorkingSetStore(nil, flusher, true, rtf.metadataNS()))
			}
		}
	}
