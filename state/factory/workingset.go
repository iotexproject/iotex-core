...
func (ws *workingSet) runAction(
	...
	if selp.Encoding() == uint32(iotextypes.Encoding_ETHEREUM_UNPROTECTED) && !g.IsDeployerWhitelisted(selp.SrcPubkey().Address()) {
		return nil, errors.Errorf("replay deployer %v not whitelisted", selp.SrcPubkey().Address().String())
	}
	// Handle action
