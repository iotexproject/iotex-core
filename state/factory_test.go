...
func TestCreateState(t *testing.T) {
	...
	require.Nil(err)
	state, _ := sf.LoadOrCreateAccountState(addr.RawAddress, big.NewInt(5))
...
func TestCandidates(t *testing.T) {
	...
	_, err = sf.LoadOrCreateAccountState(a.RawAddress, big.NewInt(100))
	require.NoError(t, err)
	_, err = sf.LoadOrCreateAccountState(b.RawAddress, big.NewInt(200))
	require.NoError(t, err)
	_, err = sf.LoadOrCreateAccountState(c.RawAddress, big.NewInt(300))
	require.NoError(t, err)
	_, err = sf.LoadOrCreateAccountState(d.RawAddress, big.NewInt(100))
	require.NoError(t, err)
	_, err = sf.LoadOrCreateAccountState(e.RawAddress, big.NewInt(100))
	require.NoError(t, err)
	_, err = sf.LoadOrCreateAccountState(f.RawAddress, big.NewInt(300))
	require.NoError(t, err)
