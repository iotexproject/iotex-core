...
func TestCandidates(t *testing.T) {
	...
	require.NoError(t, err)
	newRoot, err := ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 0, []action.Action{tx1, tx2})
	require.Nil(t, err)
	root := newRoot
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 0, []action.Action{vote})
	require.Nil(t, err)
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 1, []action.Action{vote2})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 2, []action.Action{vote3})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 3, []action.Action{tx3})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 4, []action.Action{tx4})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 5, []action.Action{vote4})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 6, []action.Action{vote5})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 7, []action.Action{vote6})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 8, []action.Action{tx5})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 9, []action.Action{vote7})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 10, []action.Action{vote8})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 11, []action.Action{vote9})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 12, []action.Action{vote10})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 13, []action.Action{vote11})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 14, []action.Action{vote12})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 15, []action.Action{vote13})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 16, []action.Action{vote14})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 17, []action.Action{tx6, tx7})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 18, []action.Action{vote15})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 19, []action.Action{vote16})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 20, []action.Action{vote17, vote18})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 21, []action.Action{tx8})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 22, []action.Action{tx9})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 23, []action.Action{tx10})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 24, []action.Action{vote19, vote20})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 25, []action.Action{vote21})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	root = newRoot
	require.Nil(t, sf.Commit(ws))
	...
	require.NoError(t, err)
	newRoot, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 26, []action.Action{vote22})
	require.Nil(t, err)
	require.NotEqual(t, newRoot, root)
	require.Nil(t, sf.Commit(ws))
	h, _ = sf.Height()
...
func TestUnvote(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 0, []action.Action{vote1})
	require.Nil(t, err)
	require.Nil(t, sf.Commit(ws))
	h, _ := sf.Height()
	cand, _ := sf.CandidatesByHeight(h)
	...
	require.NoError(t, err)
	_, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 0, []action.Action{vote2})
	require.Nil(t, err)
	require.Nil(t, sf.Commit(ws))
	h, _ = sf.Height()
	cand, _ = sf.CandidatesByHeight(h)
	...
	vote3.SetVoterPublicKey(a.PublicKey)
	require.NoError(t, err)
	_, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 0, []action.Action{vote3})
	require.Nil(t, err)
	require.Nil(t, sf.Commit(ws))
	h, _ = sf.Height()
	cand, _ = sf.CandidatesByHeight(h)
	...
	require.NoError(t, err)
	_, err = ws.RunActions(testaddress.Addrinfo["producer"].RawAddress, 0, []action.Action{vote4, vote5, vote6})
	require.Nil(t, err)
	require.Nil(t, sf.Commit(ws))
	h, _ = sf.Height()
	cand, _ = sf.CandidatesByHeight(h)
