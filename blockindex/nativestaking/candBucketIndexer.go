...
var (
	...
	_currentDeleteListKey = []byte("cdl")
	_bucketListKey        = byteutil.Uint64ToBytesBigEndian(math.MaxUint64)
)
...
func (cbi *CandBucketsIndexer) Start(ctx context.Context) error {
	...
	ret, err = cbi.kvBase.Get(db.MetadataNamespace, _currentDeleteListKey)
	switch errors.Cause(err) {
	case nil:
...
func (cbi *CandBucketsIndexer) PutBlock(ctx context.Context, blk *block.Block) error {
	...
	if newBucket > 0 || len(deletedBuckets) > 0 {
		...
		b.Put(StakingBucketsNamespace, _bucketListKey, buf, fmt.Sprintf("failed to write deleted bucket list = %d\n", h))
	}
	if b.Size() == 0 {
		return cbi.kvBase.Put(db.MetadataNamespace, _currentHeightKey, byteutil.Uint64ToBytesBigEndian(h))
	}
	// update height
...
func (cbi *CandBucketsIndexer) GetBuckets(height uint64, offset, limit uint32) (*iotextypes.VoteBucketList, uint64, error) {
	...
	ret, err := kv.Get(StakingBucketsNamespace, _bucketListKey)
