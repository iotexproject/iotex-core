...
func TestAddEndorsement(t *testing.T) {
	...
	en := NewEndorsement(cv, testaddress.Keyinfo["producer"].PubKey, testaddress.Keyinfo["producer"].PriKey, testaddress.Addrinfo["producer"].String())
	...
	cv = NewConsensusVote(hash1, 1, 2, PROPOSAL)
	en = NewEndorsement(cv, testaddress.Keyinfo["alfa"].PubKey, testaddress.Keyinfo["alfa"].PriKey, testaddress.Addrinfo["alfa"].String())
