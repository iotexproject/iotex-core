...
func TestEVM(t *testing.T) {
	...
	require.NoError(err)
	require.NoError(bc.GetFactory().Commit(nil))
...
func TestRollDice(t *testing.T) {
	...
	_, err := bc.CreateState(ta.Addrinfo["producer"].RawAddress, Gen.TotalSupply)
	require.NoError(err)
	_, err = bc.CreateState(ta.Addrinfo["alfa"].RawAddress, big.NewInt(0))
	require.NoError(err)
	_, err = bc.CreateState(ta.Addrinfo["bravo"].RawAddress, big.NewInt(12000000))
	require.NoError(err)
	_, err = bc.GetFactory().RunActions(0, nil, nil, nil, nil)
	require.NoError(err)
	require.NoError(bc.GetFactory().Commit(nil))
...
func TestERC20(t *testing.T) {
	...
	_, err = bc.CreateState(ta.Addrinfo["alfa"].RawAddress, big.NewInt(0))
	require.NoError(err)
	_, err = bc.CreateState(ta.Addrinfo["bravo"].RawAddress, big.NewInt(0))
	require.NoError(err)
	_, err = bc.GetFactory().RunActions(0, nil, nil, nil, nil)
	require.NoError(err)
	require.NoError(bc.GetFactory().Commit(nil))
