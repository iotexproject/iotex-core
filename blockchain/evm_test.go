...
func TestEVM(t *testing.T) {
	...
	execution, err = action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, contractAddr, 2, big.NewInt(0), uint64(120000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	...
	execution, err = action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, contractAddr, 3, big.NewInt(0), uint64(120000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	logger.Info().Msgf("execution %+v", execution)
	require.NoError(err)
...
func TestRollDice(t *testing.T) {
	...
	_, err := bc.CreateState(ta.Addrinfo["producer"].RawAddress, Gen.TotalSupply)
	require.NoError(err)
	data, _ := hex.DecodeString("")
	execution, err := action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, action.EmptyAddress, 1, big.NewInt(0), uint64(1000000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	require.NoError(err)
	blk, err := bc.MintNewBlock(nil, nil, []*action.Execution{execution}, ta.Addrinfo["producer"], "")
	...
	execution, err = action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, contractAddr, 2, big.NewInt(500000000), uint64(120000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	logger.Info().Msgf("execution %+v", execution)
	require.NoError(err)
...
func TestERC20(t *testing.T) {
	...
	execution, err := action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, action.EmptyAddress, 1, big.NewInt(0), uint64(10000000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	require.NoError(err)
	blk, err := bc.MintNewBlock(nil, nil, []*action.Execution{execution}, ta.Addrinfo["producer"], "")
	...
	execution, err = action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, contract, 2, big.NewInt(0), uint64(10000000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	require.NoError(err)
	...
	ex2, err := action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, contract, 3, big.NewInt(0), uint64(10000000), uint64(10), data)
	require.NoError(err)
	...
	ex3, err := action.NewExecution(
		ta.Addrinfo["alfa"].RawAddress, contract, 1, big.NewInt(0), uint64(10000000), uint64(10), data)
	require.NoError(err)
	...
	logger.Warn().Hex("v", data[:]).Msg("TestER")
	execution, err = action.NewExecution(
		ta.Addrinfo["producer"].RawAddress, contract, 4, big.NewInt(0), uint64(10000000), uint64(10), data)
	require.NoError(err)
	execution, err = execution.Sign(ta.Addrinfo["producer"])
	require.NoError(err)
	blk, err = bc.MintNewBlock(nil, nil, []*action.Execution{execution}, ta.Addrinfo["producer"], "")
