...
func injectExecution(nonceMap map[string]uint64, ap actpool.ActPool) error {
	for i := 0; i < totalActionPair; i++ {
		nonceMap[userA.String()]++
		ex1, err := action.SignedExecution(contractAddr, priKeyA, nonceMap[userA.String()], big.NewInt(0), 2e6, big.NewInt(testutil.TestGasPriceInt64), opAppend)
		if err != nil {
			return err
		}
		...
		nonceMap[userB.String()]++
		ex2, err := action.SignedExecution(contractAddr, priKeyB, nonceMap[userB.String()], big.NewInt(0), 2e6, big.NewInt(testutil.TestGasPriceInt64), opAppend)
		if err != nil {
			return err
		}
