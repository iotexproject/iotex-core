...
func TestProposalPool(t *testing.T) {
	...
	t.Run("addblock", func(t *testing.T) {
		...
		pool.Init(block.GenesisBlock().HashBlock())
		blk, err := mint(block.GenesisBlock(), 0)
		r.NoError(err)
		r.NoError(pool.AddBlock(blk))
		r.Equal(blk, pool.Block(blk.Height()))
		r.Equal(blk, pool.BlockByHash(blk.HashBlock()))
		r.Equal(blk.Height(), pool.Tip())
	})
	t.Run("add-dangling-block", func(t *testing.T) {
		pool := newProposalPool()
		pool.Init(block.GenesisBlock().HashBlock())
		blk, err := mint(block.GenesisBlock(), 0)
		r.NoError(err)
		dangling, err := mint(blk, 0)
		r.NoError(err)
		r.Error(pool.AddBlock(dangling))
		r.Nil(pool.Block(dangling.Height()))
		r.Nil(pool.BlockByHash(dangling.HashBlock()))
	})
	t.Run("1-fork", func(t *testing.T) {
		...
		r.Equal(blocks[len(blocks)-1].Height(), pool.Tip())
		r.Equal(confirm.HashBlock(), pool.root)
		for i := range blocks {
			blk := blocks[i]
			if blk.Height() <= confirm.Height() {
				r.Nil(pool.Block(blk.Height()))
			} else {
				r.Equal(blk, pool.Block(blk.Height()))
			}
		}
	...
	t.Run("forks", func(t *testing.T) {
		...
		r.Equal(blocks[7].Height(), pool.Tip())
		confirm := blocks[6]
