...
func TestBlockchain_MintNewBlock(t *testing.T) {
	...
	elp2, err := bd.SetAction(execution).
		SetNonce(2).
		SetGasLimit(100000).
		SetGasPrice(big.NewInt(10)).Build()
	require.NoError(t, err)
...
func TestBlockchain_AccountState(t *testing.T) {
	...
	sf, err := factory.NewFactory(cfg, factory.InMemTrieOption(), factory.RegistryOption(registry))
	require.NoError(err)
	bc := NewBlockchain(cfg, nil, sf, InMemDaoOption(sf))
	require.NoError(bc.Start(context.Background()))
	require.NotNil(bc)
...
func TestBlocks(t *testing.T) {
	...
	acc := account.NewProtocol(rewarding.DepositGas)
	require.NoError(acc.Register(registry))
	sf, _ := factory.NewFactory(cfg, factory.InMemTrieOption(), factory.RegistryOption(registry))

	// Create a blockchain from scratch
	bc := NewBlockchain(cfg, nil, sf, BoltDBDaoOption(sf))
	require.NoError(bc.Start(context.Background()))
	defer func() {
		require.NoError(bc.Stop(context.Background()))
	}()
...
func makeTransfer(contract string, bc Blockchain, t *testing.T) *block.Block {
	...
	require.NoError(err)
	execution, err := action.NewExecution(contract, big.NewInt(0), bytecode)
	require.NoError(err)
	bd := &action.EnvelopeBuilder{}
	elp, err := bd.SetAction(execution).
		SetNonce(4).
