...
func TestWrongNonce(t *testing.T) {
	...
	require.NoError(action.Sign(tsf1, ta.Addrinfo["producer"].PrivateKey))
	hash := tsf1.Hash()
	blk := NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf, tsf1}, nil, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	require.Nil(val.Validate(blk, 2, hash, true))
	...
	require.NoError(action.Sign(tsf2, ta.Addrinfo["producer"].PrivateKey))
	hash = tsf1.Hash()
	blk = NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf, tsf1, tsf2}, nil, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	...
	hash = tsf1.Hash()
	blk = NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf}, []*action.Vote{vote}, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	err = val.Validate(blk, 2, hash, true)
	require.Error(err)
	...
	hash = tsf1.Hash()
	blk = NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf, tsf3, tsf4}, nil, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	err = val.Validate(blk, 2, hash, true)
	require.Error(err)
	...
	hash = tsf1.Hash()
	blk = NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf}, []*action.Vote{vote2, vote3}, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	err = val.Validate(blk, 2, hash, true)
	require.Error(err)
	...
	hash = tsf1.Hash()
	blk = NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf, tsf5, tsf6}, nil, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	err = val.Validate(blk, 2, hash, true)
	require.Error(err)
	...
	hash = tsf1.Hash()
	blk = NewBlock(chainID, 3, hash, testutil.TimestampNow(), []*action.Transfer{coinbaseTsf}, []*action.Vote{vote4, vote5}, nil)
	err = blk.SignBlock(ta.Addrinfo["producer"])
	require.NoError(err)
	err = val.Validate(blk, 2, hash, true)
	require.Error(err)
