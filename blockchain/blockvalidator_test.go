...
func TestWrongNonce(t *testing.T) {
	...
	sf.AddActionHandlers(account.NewProtocol(hc))
	...
	val.AddActionValidators(account.NewProtocol(hc))
...
func TestWrongAddress(t *testing.T) {
	...
	hc := config.NewHeightUpgrade(cfg)
	bc.GetFactory().AddActionHandlers(account.NewProtocol(hc))
	...
	val.AddActionEnvelopeValidators(protocol.NewGenericValidator(bc))
	val.AddActionValidators(account.NewProtocol(hc), execution.NewProtocol(bc, hc))
