...
type BlockHeader struct {
	...
	DKGSecrets    [][]uint32        // dkg secrets generated by producer
	DKGWitness    [][]byte          // dkg witness generated by producer
}
...
func (b *Block) ByteStreamHeader() []byte {
	...
	for i := 0; i < len(b.Header.DKGSecrets); i++ {
		for j := 0; j < len(b.Header.DKGSecrets[i]); j++ {
			tmp4B := make([]byte, 4)
			enc.MachineEndian.PutUint32(tmp4B, b.Header.DKGSecrets[i][j])
			stream = append(stream, tmp4B...)
		}
	}
	for i := 0; i < len(b.Header.DKGWitness); i++ {
		stream = append(stream, b.Header.DKGWitness[i]...)
	}
	return stream
}
...
func (b *Block) ConvertToBlockHeaderPb() *iproto.BlockHeaderPb {
	...
	for _, secret := range b.Header.DKGSecrets {
		pbHeader.DkgSecrets = append(pbHeader.DkgSecrets, &iproto.DKGSecret{DkgSecret: secret})
	}
	pbHeader.DkgWitness = append(pbHeader.DkgWitness, b.Header.DKGWitness...)
	return &pbHeader
}
...
func (b *Block) ConvertFromBlockHeaderPb(pbBlock *iproto.BlockPb) {
	...
	dkgSecret := [][]uint32{}
	for _, secret := range pbBlock.GetHeader().GetDkgSecrets() {
		dkgSecret = append(dkgSecret, secret.DkgSecret)
	}
	b.Header.DKGSecrets = dkgSecret
	b.Header.DKGWitness = pbBlock.GetHeader().GetDkgWitness()
}
