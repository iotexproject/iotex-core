...
func TestBlockDAO(t *testing.T) {
	...
	testDeleteDao := func(kvstore db.KVStore, indexer blockindex.Indexer, t *testing.T) {
		...
		for i := range daoTests {
			...
			prevHeight, err := dao.GetTipHeight()
			require.NoError(err)
			prevHash, err := dao.GetBlockHash(prevHeight)
			require.NoError(err)
			require.NoError(dao.DeleteTipBlock())
			tipHeight, err := indexer.GetBlockchainHeight()
			require.NoError(err)
			require.EqualValues(prevHeight - 1, tipHeight)
			...
			require.NoError(err)
			require.Equal(h, h1)
			_, err = dao.GetBlockHash(prevHeight)
			require.Error(err)
			_, err = dao.GetBlockHeight(prevHash)
			require.Error(err)
			if i <= 2 {
