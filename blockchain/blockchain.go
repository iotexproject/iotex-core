...
func (bc *blockchain) commitBlock(blk *block.Block) error {
	log.L().Debug("blockchain.commitBlock: start", zap.Uint64("height", blk.Height()))
	t0 := time.Now()
	tipHeight, err := bc.dao.Height()
	if err != nil {
		return err
	...
	ctx = protocol.WithFeatureCtx(ctx)
	// write block into DB
	log.L().Debug("blockchain.commitBlock: calling dao.PutBlock", zap.Uint64("height", blk.Height()))
	putTimer := bc.timerFactory.NewTimer("putBlock")
	err = bc.dao.PutBlock(ctx, blk)
	putTimer.End()
	log.L().Debug("blockchain.commitBlock: dao.PutBlock done", zap.Uint64("height", blk.Height()), zap.Duration("elapsed", time.Since(t0)), zap.Error(err))
	...
	log.L().Debug("blockchain.commitBlock: calling persistPendingWitness", zap.Uint64("height", blk.Height()))
	tWitness := time.Now()
	if err := bc.persistPendingWitness(blk); err != nil {
		return err
	}
	log.L().Debug("blockchain.commitBlock: persistPendingWitness done", zap.Uint64("height", blk.Height()), zap.Duration("elapsed", time.Since(tWitness)))
	...
	log.L().Debug("blockchain.commitBlock: calling emitToSubscribers", zap.Uint64("height", blk.Height()))
	tEmit := time.Now()
	bc.emitToSubscribers(blk)
	log.L().Debug("blockchain.commitBlock: done", zap.Uint64("height", blk.Height()), zap.Duration("emitElapsed", time.Since(tEmit)), zap.Duration("totalElapsed", time.Since(t0)))
	return nil
}
