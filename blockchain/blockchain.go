...
func (bc *blockchain) GetActionsFromAddress(addrStr string) ([]hash.Hash256, error) {
	...
	return blockdao.GetActionsBySenderAddress(bc.dao.KVStore(), hash.BytesToHash160(addr.Bytes()))
}
...
func (bc *blockchain) GetActionsToAddress(addrStr string) ([]hash.Hash256, error) {
	...
	return blockdao.GetActionsByRecipientAddress(bc.dao.KVStore(), hash.BytesToHash160(addr.Bytes()))
}
...
func (bc *blockchain) GetActionCountByAddress(addrStr string) (uint64, error) {
	...
	fromCount, err := blockdao.GetActionCountBySenderAddress(bc.dao.KVStore(), hash.BytesToHash160(addr.Bytes()))
	if err != nil {
		return 0, err
	}
	toCount, err := blockdao.GetActionCountByRecipientAddress(bc.dao.KVStore(), hash.BytesToHash160(addr.Bytes()))
	if err != nil {
		return 0, err
	}
...
func (bc *blockchain) getActionByActionHashHelper(h hash.Hash256) (hash.Hash256, error) {
	return blockdao.GetBlockHashByActionHash(bc.dao.KVStore(), h)
}
