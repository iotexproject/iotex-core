...
func TestTxQueue_Put(t *testing.T) {
	...
	tx1 := trx.Transfer{Nonce: uint64(2), Amount: big.NewInt(10)}
	q.Put(&tx1)
	assert.Equal(uint64(2), q.index[0])
	assert.NotNil(q.items[tx1.Nonce])
	tx2 := trx.Transfer{Nonce: uint64(1), Amount: big.NewInt(100)}
	q.Put(&tx2)
	...
	tx3 := trx.Transfer{Nonce: uint64(1), Amount: big.NewInt(1000)}
	err := q.Put(&tx3)
	assert.NotNil(err)
}
...
func TestTxQueue_FilterNonce(t *testing.T) {
	assert := assert.New(t)
	q := NewTxQueue().(*txQueue)
	tx1 := trx.Transfer{Nonce: uint64(1), Amount: big.NewInt(1)}
	tx2 := trx.Transfer{Nonce: uint64(2), Amount: big.NewInt(100)}
	tx3 := trx.Transfer{Nonce: uint64(3), Amount: big.NewInt(1000)}
	q.Put(&tx1)
	q.Put(&tx2)
	q.Put(&tx3)
...
func TestTxQueue_UpdateNonce(t *testing.T) {
	...
	q := NewTxQueue().(*txQueue)
	tx1 := trx.Transfer{Nonce: uint64(1), Amount: big.NewInt(1)}
	tx2 := trx.Transfer{Nonce: uint64(3), Amount: big.NewInt(1000)}
	tx3 := trx.Transfer{Nonce: uint64(4), Amount: big.NewInt(10000)}
	tx4 := trx.Transfer{Nonce: uint64(6), Amount: big.NewInt(100000)}
	tx5 := trx.Transfer{Nonce: uint64(2), Amount: big.NewInt(100)}
	q.Put(&tx1)
	q.Put(&tx2)
	q.Put(&tx3)
	q.Put(&tx4)
...
func TestTxQueue_ConfirmedTxs(t *testing.T) {
	assert := assert.New(t)
	q := NewTxQueue().(*txQueue)
	tx1 := trx.Transfer{Nonce: uint64(2), Amount: big.NewInt(1)}
	tx2 := trx.Transfer{Nonce: uint64(3), Amount: big.NewInt(100)}
	tx3 := trx.Transfer{Nonce: uint64(4), Amount: big.NewInt(1000)}
	tx4 := trx.Transfer{Nonce: uint64(6), Amount: big.NewInt(10000)}
	tx5 := trx.Transfer{Nonce: uint64(7), Amount: big.NewInt(100000)}
	q.Put(&tx1)
	q.Put(&tx2)
	q.Put(&tx3)
	q.Put(&tx4)
	...
	q.confirmedNonce = 4
	txs := q.ConfirmedTxs()
	assert.Equal([]*trx.Transfer{&tx1, &tx2}, txs)
}
