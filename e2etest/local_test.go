...
func TestLocalCommit(t *testing.T) {
	...
	require.NoError(err)
	sgdDBPath, err := testutil.PathOfTempFile(_dBPath)
	require.NoError(err)
	cfg.Chain.TrieDBPatchFile = ""
	cfg.Chain.TrieDBPath = testTriePath
	cfg.Chain.ChainDBPath = testDBPath
	cfg.Chain.IndexDBPath = indexDBPath
	cfg.Chain.SGDIndexDBPath = sgdDBPath
	defer func() {
		testutil.CleanupPath(testTriePath)
		testutil.CleanupPath(testDBPath)
		testutil.CleanupPath(indexDBPath)
		testutil.CleanupPath(sgdDBPath)
	}()
...
func TestLocalSync(t *testing.T) {
	...
	indexDBPath2, err := testutil.PathOfTempFile(_dBPath2)
	require.NoError(err)
	sgdDBPath2, err := testutil.PathOfTempFile(_dBPath2)
	require.NoError(err)
	...
	cfg.Chain.ChainDBPath = testDBPath2
	cfg.Chain.IndexDBPath = indexDBPath2
	cfg.Chain.SGDIndexDBPath = sgdDBPath2
	defer func() {
		testutil.CleanupPath(testTriePath2)
		testutil.CleanupPath(testDBPath2)
		testutil.CleanupPath(indexDBPath2)
		testutil.CleanupPath(sgdDBPath2)
	}()
...
func TestStartExistingBlockchain(t *testing.T) {
	...
	require.NoError(err)
	testSGDPath, err := testutil.PathOfTempFile(_dBPath)
	require.NoError(err)
