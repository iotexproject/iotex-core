...
func newConfigForNodeInfoTest(triePath, dBPath, idxDBPath, contracStakeIdxDBPath string) (config.Config, func(), error) {
	...
	contractStakeIdxDBPath, err := testutil.PathOfTempFile(contracStakeIdxDBPath)
	if err != nil {
		return cfg, nil, err
	}
	...
	cfg.Chain.ContractStakingIndexDBPath = contractStakeIdxDBPath
	return cfg, func() {
		testutil.CleanupPath(testTriePath)
		testutil.CleanupPath(testDBPath)
		testutil.CleanupPath(indexDBPath)
		testutil.CleanupPath(contractStakeIdxDBPath)
	}, nil
}
...
func TestBroadcastNodeInfo(t *testing.T) {
	...
	cfgSender, teardown, err := newConfigForNodeInfoTest("trie.test", "db.test", "indexdb.test", "stake.test")
	require.NoError(err)
	defer teardown()
	...
	cfgReciever, teardown2, err := newConfigForNodeInfoTest("trie2.test", "db2.test", "indexdb2.test", "stake2.test")
	require.NoError(err)
	defer teardown2()
