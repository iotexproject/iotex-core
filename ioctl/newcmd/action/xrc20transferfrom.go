...
import (
	...
	"github.com/iotexproject/iotex-core/v2/ioctl/newcmd/alias"
)
...
func NewXrc20TransferFromCmd(client ioctl.Client) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			contractAddr, err := cmd.Flags().GetString("contract-address")
			if err != nil {
				return errors.Wrap(err, "failed to get contract address")
			}
			contract, err := xrc20Contract(client, contractAddr)
			if err != nil {
				return errors.Wrap(err, "failed to get contract address")
			}
