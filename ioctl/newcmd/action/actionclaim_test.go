...
func TestNewActionClaimCmd(t *testing.T) {
	...
	client.EXPECT().AskToConfirm(gomock.Any()).Return(true, nil).Times(3)
	...
	apiServiceClient.EXPECT().GetAccount(gomock.Any(), gomock.Any()).Return(accountResp, nil).Times(6)
	apiServiceClient.EXPECT().GetChainMeta(gomock.Any(), gomock.Any()).Return(chainMetaResp, nil).Times(3)
	apiServiceClient.EXPECT().SendAction(gomock.Any(), gomock.Any()).Return(sendActionResp, nil).Times(3)
	...
	t.Run("action claim with address", func(t *testing.T) {
		address := "0x123"
		cmd := NewActionClaimCmd(client)
		result, err := util.ExecuteCmd(cmd, "0", address, "--gas-limit", "0")
		require.NoError(err)
		require.Contains(result, "Action has been sent to blockchain")
	})
	...
	t.Run("action claim with payload", func(t *testing.T) {
		...
		cmd := NewActionClaimCmd(client)
		result, err := util.ExecuteCmd(cmd, "0", "0x123", payload, "--gas-limit", "0")
		require.NoError(err)
		require.Contains(result, "Action has been sent to blockchain")
	})
