...
func TestNewActionDepositCmd(t *testing.T) {
	...
	t.Run("action deposite", func(t *testing.T) {
		cmd := NewActionDepositCmd(client)
		result, err := util.ExecuteCmd(cmd, "0")
		require.NoError(err)
		require.Contains(result, "Action has been sent to blockchain")
	})

	t.Run("action deposite with payload", func(t *testing.T) {
