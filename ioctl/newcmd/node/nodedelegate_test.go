...
func TestNewNodeDelegateCmd(t *testing.T) {
	...
	apiServiceClient.EXPECT().ReadState(gomock.Any(), gomock.Any()).Return(probationList, nil).Times(3)
	...
	t.Run("get zero delegate epoch", func(t *testing.T) {
		...
		apiServiceClient.EXPECT().GetEpochMeta(gomock.Any(), gomock.Any()).Return(epochMetaResponse, nil)
