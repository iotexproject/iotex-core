...
func TestActPool_AddActs(t *testing.T) {
	...
	// Test actpool status after adding a sequence of Tsfs:
	// need to check confirmed nonce, pending nonce, and pending balance
...
func TestActPool_PickActs(t *testing.T) {
	createActPool := func(cfg config.ActPool) (*actPool, []action.SealedEnvelope, []action.SealedEnvelope, []action.SealedEnvelope) {
		...
		require.True(ok)
		ap.AddActionEnvelopeValidators(protocol.NewGenericValidator(bc, genesis.Default.ActionGasLimit))
		ap.AddActionValidators(account.NewProtocol())
...
func TestActPool_removeConfirmedActs(t *testing.T) {
	...
	require.Equal(3, len(ap.allActions))
	...
	_, err = ws.RunActions(ctx, 0, []action.SealedEnvelope{tsf1, tsf2, tsf3})
	require.NoError(err)
	require.Nil(sf.Commit(ws))
...
func TestActPool_removeInvalidActs(t *testing.T) {
	...
	hash1 := tsf1.Hash()
	acts := []action.SealedEnvelope{tsf1}
...
func TestActPool_GetUnconfirmedActs(t *testing.T) {
	...
	require.Equal([]action.SealedEnvelope{tsf1, tsf3}, acts)
}
...
func TestActPool_GetSize(t *testing.T) {
	...
	require.Equal(uint64(3), ap.GetSize())
	require.Equal(uint64(30000), ap.GetGasSize())
	sf := bc.GetFactory()
	require.NotNil(sf)
	ws, err := sf.NewWorkingSet()
	require.NoError(err)
