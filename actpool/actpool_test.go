...
func TestActPool_NewActPool(t *testing.T) {
	...
	_, err := NewActPool(genesis.TestConfig(), nil, DefaultConfig, nil)
	require.Error(err)

	// all good
	require.Panics(func() { blockchain.NewBlockchain(blockchain.DefaultConfig, genesis.TestConfig(), nil, nil, nil) }, "option is nil")
	sf := mock_chainmanager.NewMockStateReader(ctrl)
	act, err := NewActPool(genesis.TestConfig(), sf, DefaultConfig)
	require.NoError(err)
	require.NotNil(act)

	// panic caused by option is nil
	require.Panics(func() { NewActPool(genesis.TestConfig(), sf, DefaultConfig, nil) }, "option is nil")

	// error caused by option
	opt2 := func(pool *actPool) error {
		return errors.New("test error")
	}
	_, err = NewActPool(genesis.TestConfig(), sf, DefaultConfig, opt2)
	require.Error(err)
...
func TestActPool_AddActs(t *testing.T) {
	...
	// Create actpool
	apConfig := getActPoolCfg()
	Ap, err := NewActPool(genesis.TestConfig(), sf, apConfig)
	require.NoError(err)
	ap, ok := Ap.(*actPool)
	require.True(ok)
	...
	Ap2, err := NewActPool(genesis.TestConfig(), sf, apConfig)
	require.NoError(err)
	ap2, ok := Ap2.(*actPool)
	require.True(ok)
	...
	Ap3, err := NewActPool(genesis.TestConfig(), sf, apConfig)
	require.NoError(err)
	ap3, ok := Ap3.(*actPool)
	require.True(ok)
...
func TestActPool_PickActs(t *testing.T) {
	...
	createActPool := func(cfg Config) (*actPool, []action.SealedEnvelope, []action.SealedEnvelope, []action.SealedEnvelope) {
		// Create actpool
		Ap, err := NewActPool(genesis.TestConfig(), sf, cfg)
		require.NoError(err)
		ap, ok := Ap.(*actPool)
		require.True(ok)
		ap.AddActionEnvelopeValidators(protocol.NewGenericValidator(sf, accountutil.AccountState))
...
func TestActPool_Reset(t *testing.T) {
	...
	apConfig := getActPoolCfg()
	Ap1, err := NewActPool(genesis.TestConfig(), sf, apConfig)
	require.NoError(err)
	ap1, ok := Ap1.(*actPool)
	require.True(ok)
...
func getPendingBalance(ap *actPool, addrStr string) (*big.Int, error) {
	...
	state, err := accountutil.AccountState(genesis.WithGenesisContext(context.Background(), genesis.TestConfig()), ap.sf, addr)
	if err != nil {
		return nil, err
	}
