...
func TestActPool_AddActs(t *testing.T) {
	...
	require.True(ok)
	ap.AddActionEnvelopeValidators(protocol.NewGenericValidator(bc))
	ap.AddActionValidators(account.NewProtocol(0), execution.NewProtocol(bc, config.NewHeightChange(config.Default)))
...
func TestActPool_removeConfirmedActs(t *testing.T) {
	...
	bc.GetFactory().AddActionHandlers(account.NewProtocol(0), execution.NewProtocol(bc, config.NewHeightChange(config.Default)))
	require.NoError(bc.Start(context.Background()))
	_, err := bc.CreateState(addr1, big.NewInt(100))
	require.NoError(err)
	// Create actpool
...
func TestActPool_Reset(t *testing.T) {
	...
	ap1.AddActionValidators(account.NewProtocol(0), execution.NewProtocol(bc, config.NewHeightChange(config.Default)))
	Ap2, err := NewActPool(bc, apConfig, EnableExperimentalActions())
	require.NoError(err)
	ap2, ok := Ap2.(*actPool)
	require.True(ok)
	ap2.AddActionEnvelopeValidators(protocol.NewGenericValidator(bc))
	ap2.AddActionValidators(account.NewProtocol(0), execution.NewProtocol(bc, config.NewHeightChange(config.Default)))
