...
func (bp *BundlePool) AddBundle(ctx context.Context, sender address.Address, uuid string, bundle *action.Bundle) error {
	...
	// System actions are generated by the block producer at mint time and
	// must never enter the bundle pool; accepting one would let a remote
	// peer inject state-changing system actions into every proposer's mint
	// path.
	if err := bundle.ForEach(func(selp *action.SealedEnvelope) error {
		if action.IsSystemAction(selp) {
			return errors.Errorf("system action %T is not allowed in a bundle", selp.Action())
		}
		return nil
	}); err != nil {
