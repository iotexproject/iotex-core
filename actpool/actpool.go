...
func (ap *actPool) GetPendingNonce(addr string) (uint64, error) {
	...
	accountAddr, err := address.FromString(addr)
	if err != nil {
		return 0, err
	}
	confirmedState, err := accountutil.AccountState(ap.sf, accountAddr)
	if err != nil {
		return 0, err
	}
...
func (ap *actPool) enqueueAction(sender string, act action.SealedEnvelope, actHash hash.Hash256, actNonce uint64) error {
	...
	confirmedState, err := accountutil.AccountState(ap.sf, senderAddr)
	if err != nil {
	...
	if queue == nil {
		...
		state, err := accountutil.AccountState(ap.sf, senderAddr)
		if err != nil {
...
func (ap *actPool) removeConfirmedActs() {
	for from, queue := range ap.accountActs {
		...
		if err != nil {
			log.L().Error("Error when getting address from string", zap.Error(err))
			return
		}
		confirmedState, err := accountutil.AccountState(ap.sf, fromAddr)
		if err != nil {
...
func (ap *actPool) reset() {
	...
	for from, queue := range ap.accountActs {
		...
		fromAddr, err := address.FromString(from)
		if err != nil {
			log.L().Error("Error when getting address from encoded string.", zap.Error(err))
			return
		}
		state, err := accountutil.AccountState(ap.sf, fromAddr)
		if err != nil {
