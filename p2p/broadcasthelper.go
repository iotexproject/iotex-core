...
var maxMessageBodySize=104755  //1*1024*1024-1024
...
func (h*broadcastHelperHeap) AddMessage(msg*p2ppb.BroadcastMsg) *p2ppb.BroadcastMsg{
	defer func(){
		//remove message that is too old
		for len(h.bhs)>0{
			bh:=h.bhs[0]
			//remove item that is arrived at 1 minute ago
			if bh.t.Before(time.Now().Add(0-time.Minute)) {
