...
func TestProtocol_Handle(t *testing.T) {
	...
	require.NoError(err)
	require.NoError(protocol.Handle(vote1, ws))
	...
	require.NoError(err)
	require.NoError(protocol.Handle(vote2, ws))
	...
	require.NoError(err)
	require.NoError(protocol.Handle(vote3, ws))
	...
	require.NoError(err)
	require.NoError(protocol.Handle(unvote1, ws))
	account1, _ = account.LoadAccountState(ws, pkHash1)
	...
	require.NoError(err)
	require.NoError(protocol.Handle(vote4, ws))
	account2, _ = account.LoadAccountState(ws, pkHash2)
	account3, _ = account.LoadAccountState(ws, pkHash3)
	...
	require.NoError(err)
	require.NoError(protocol.Handle(unvote2, ws))
	account2, _ = account.LoadAccountState(ws, pkHash2)
	account3, _ = account.LoadAccountState(ws, pkHash3)
