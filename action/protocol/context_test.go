...
func TestWithRunActionsCtx(t *testing.T) {
	...
	require.NoError(err)
	actionCtx := RunActionsCtx{1, time.Now(), 1, addr, addr, hash.ZeroHash256, nil, 0, 0, false, nil}
...
func TestGetRunActionsCtx(t *testing.T) {
	...
	addr, err := address.FromString("io1mflp9m6hcgm2qcghchsdqj3z3eccrnekx9p0ms")
	require.NoError(err)
	actionCtx := RunActionsCtx{1111, time.Now(), 1, addr, addr, hash.ZeroHash256, nil, 0, 0, false, nil}
