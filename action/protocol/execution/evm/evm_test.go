...
func TestGasEstimate(t *testing.T) {
	...
	for _, v := range []struct {
		...
		{config.Default.Genesis.BlockGasLimit, 8200300, make([]byte, 20000)},
		{1000000, 245600, make([]byte, 5600)},
		{500000, 21000, make([]byte, 36)},
	} {
		evmLeftOver, remainingGas, err := gasExecuteInEVM(v.gas, v.consume, v.data)
		require.NoError(err)
		gasConsumed := v.gas - remainingGas
		require.Equal(v.gas-evmLeftOver, 2*gasConsumed)

		// if we use 2*gasConsumed, EVM will consume the exact amount of gas
		evmLeftOver, remainingGas, err = gasExecuteInEVM(2*gasConsumed, v.consume, v.data)
		require.NoError(err)
...
func gasExecuteInEVM(gas, consume uint64, data []byte) (uint64, uint64, error) {
	remainingGas := gas
