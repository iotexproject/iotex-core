...
func (sct *smartContractTest) deployContract(
	...
	h, _ := iotxaddress.GetPubkeyHash(contractAddress)
...
func TestProtocol_Handle(t *testing.T) {
	testEVM := func(t *testing.T) {
		...
		ws, err := sf.NewWorkingSet()
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["producer"].RawAddress, blockchain.Gen.TotalSupply)
		require.NoError(err)
		gasLimit := testutil.TestGasLimit
		ctx = protocol.WithRunActionsCtx(ctx,
			protocol.RunActionsCtx{
		...
		execution, err := action.NewExecution(
			ta.IotxAddrinfo["producer"].RawAddress, action.EmptyAddress, 1, big.NewInt(0), uint64(100000), big.NewInt(0), data)
		require.NoError(err)
		...
		actionMap := make(map[string][]action.SealedEnvelope)
		actionMap[selp.SrcAddr()] = []action.SealedEnvelope{selp}
		blk, err := bc.MintNewBlock(actionMap, ta.IotxAddrinfo["producer"].PublicKey, ta.IotxAddrinfo["producer"].PrivateKey, ta.IotxAddrinfo["producer"].RawAddress, nil, nil, "")
		require.NoError(err)
		...
		exes, err := bc.GetExecutionsFromAddress(ta.IotxAddrinfo["producer"].RawAddress)
		require.Nil(err)
		...
		execution, err = action.NewExecution(
			ta.IotxAddrinfo["producer"].RawAddress, contractAddr, 2, big.NewInt(0), uint64(120000), big.NewInt(0), data)
		require.NoError(err)
		...
		execution1, err := action.NewExecution(
			ta.IotxAddrinfo["producer"].RawAddress, action.EmptyAddress, 4, big.NewInt(0), uint64(100000), big.NewInt(10), data)
		require.NoError(err)
		bd = &action.EnvelopeBuilder{}
		...
		actionMap = make(map[string][]action.SealedEnvelope)
		actionMap[selp.SrcAddr()] = []action.SealedEnvelope{selp}
		blk, err = bc.MintNewBlock(actionMap, ta.IotxAddrinfo["alfa"].PublicKey, ta.IotxAddrinfo["alfa"].PrivateKey, ta.IotxAddrinfo["alfa"].RawAddress, nil, nil, "")
		require.NoError(err)
		require.NoError(bc.ValidateBlock(blk, true))
		require.Nil(bc.CommitBlock(blk))
		require.Equal(1, len(blk.Receipts))
		ws, _ = sf.NewWorkingSet()
		alfaAccount, _ := account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["alfa"].RawAddress, blockchain.Gen.TotalSupply)
	...
	testRollDice := func(t *testing.T) {
		...
		_, err = account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["producer"].RawAddress, blockchain.Gen.TotalSupply)
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["alfa"].RawAddress, big.NewInt(0))
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["bravo"].RawAddress, big.NewInt(12000000))
		require.NoError(err)
		gasLimit := testutil.TestGasLimit
		ctx = protocol.WithRunActionsCtx(ctx,
			protocol.RunActionsCtx{
		...
		execution, err := action.NewExecution(
			ta.IotxAddrinfo["producer"].RawAddress, action.EmptyAddress, 1, big.NewInt(0), uint64(1000000), big.NewInt(0), data)
		require.NoError(err)
		...
		execution, err = action.NewExecution(
			ta.IotxAddrinfo["producer"].RawAddress, contractAddr, 2, big.NewInt(500000000), uint64(120000), big.NewInt(0), data)
		require.NoError(err)
		...
		// verify balance
		balance, err = bc.Balance(ta.IotxAddrinfo["alfa"].RawAddress)
		require.NoError(err)
		...
		execution, err = action.NewExecution(
			ta.IotxAddrinfo["bravo"].RawAddress, contractAddr, 1, big.NewInt(0), uint64(120000), big.NewInt(10), data)
		require.NoError(err)
	...
	testERC20 := func(t *testing.T) {
		...
		_, err = account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["alfa"].RawAddress, big.NewInt(0))
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, ta.IotxAddrinfo["bravo"].RawAddress, big.NewInt(0))
		require.NoError(err)
		gasLimit := testutil.TestGasLimit
		ctx = protocol.WithRunActionsCtx(ctx,
			protocol.RunActionsCtx{
		...
		execution, err := action.NewExecution(
			ta.IotxAddrinfo["producer"].RawAddress, action.EmptyAddress, 1, big.NewInt(0), uint64(10000000), big.NewInt(0), data)
		require.NoError(err)
		...
		to, _ = iotxaddress.GetPubkeyHash(ta.IotxAddrinfo["bravo"].RawAddress)
		bravo.SetBytes(to)
		value = hash.ZeroHash32B
		h = value[24:]
...
func TestERC20(t *testing.T) {
	sct := &smartContractTest{
		prepare: map[string]*big.Int{
			ta.Addrinfo["producer"].IotxAddress(): blockchain.Gen.TotalSupply,
			ta.Addrinfo["alfa"].IotxAddress():     big.NewInt(0),
			ta.Addrinfo["bravo"].IotxAddress():    big.NewInt(0),
		},
		deploy: execCfg{
			...
			executor:         ta.IotxAddrinfo["producer"].RawAddress,
			privateKey:       ta.IotxAddrinfo["producer"].PrivateKey,
			amount:           0,
		...
		executions: []execCfg{
			...
			func() execCfg {
				data, _ := hex.DecodeString("a9059cbb")
				bravo := hash.ZeroHash32B
				to, _ := iotxaddress.GetPubkeyHash(ta.Addrinfo["bravo"].IotxAddress())
				bravo.SetBytes(to)
				value := hash.ZeroHash32B
				h := value[24:]
				binary.BigEndian.PutUint64(h, 20000)
			...
			func() execCfg {
				...
				data = append(data, value[:]...)
				return execCfg{
					executor:         ta.IotxAddrinfo["alfa"].RawAddress,
					privateKey:       ta.IotxAddrinfo["alfa"].PrivateKey,
					codeHex:          hex.EncodeToString(data),
					amount:           0,
					gasLimit:         uint64(10000000),
					gasPrice:         0,
			...
			func() execCfg {
				data, _ := hex.DecodeString("70a08231")
				alfa := hash.ZeroHash32B
				to, _ := iotxaddress.GetPubkeyHash(ta.Addrinfo["alfa"].IotxAddress())
				alfa.SetBytes(to)
				data = append(data, alfa[:]...)
...
func TestDelegateERC20(t *testing.T) {
	sct := &smartContractTest{
		prepare: map[string]*big.Int{ta.IotxAddrinfo["alfa"].RawAddress: big.NewInt(9876543210)},
		deploy: execCfg{
			executor:            ta.IotxAddrinfo["alfa"].RawAddress,
			privateKey:          ta.IotxAddrinfo["alfa"].PrivateKey,
