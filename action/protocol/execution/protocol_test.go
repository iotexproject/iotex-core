...
func TestProtocol_Handle(t *testing.T) {
	testEVM := func(t *testing.T) {
		...
		ws, err := sf.NewWorkingSet()
		require.NoError(err)
		_, err = accountutil.LoadOrCreateAccount(ws, identityset.Addrinfo["producer"].String(), unit.ConvertIotxToRau(1000000000))
		require.NoError(err)
		...
		actionMap := make(map[string][]action.SealedEnvelope)
		actionMap[identityset.Addrinfo["producer"].String()] = []action.SealedEnvelope{selp}
		blk, err := bc.MintNewBlock(
			actionMap,
			testutil.TimestampNow(),
		)
		...
		exes, err := bc.GetActionsFromAddress(identityset.Addrinfo["producer"].String())
		require.Nil(err)
...
func TestProtocol_Validate(t *testing.T) {
	...
	ex, err = action.NewExecution(
		identityset.Addrinfo["bravo"].String()+"bbb",
		uint64(1),
		big.NewInt(0),
		uint64(0),
		big.NewInt(0),
