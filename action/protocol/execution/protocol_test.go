...
func runExecution(
	...
	blk, err := bc.MintNewBlock(
		actionMap,
		&testaddress.Keyinfo["producer"].PublicKey,
		testaddress.Keyinfo["producer"],
...
func TestProtocol_Handle(t *testing.T) {
	testEVM := func(t *testing.T) {
		...
		selp, err := action.Sign(elp, testaddress.Addrinfo["producer"].Bech32(), testaddress.Keyinfo["producer"])
		require.NoError(err)
	...
	testRollDice := func(t *testing.T) {
		...
		selp, err = action.Sign(elp, testaddress.Addrinfo["bravo"].Bech32(), testaddress.Keyinfo["bravo"])
		require.NoError(err)
		log.S().Infof("execution %+v\n", execution)
	...
	testERC20 := func(t *testing.T) {
		...
		execution, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), action.EmptyAddress, 1, big.NewInt(0), uint64(5000000), big.NewInt(0), data)
		require.NoError(err)
		...
		execution, err = action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), contract, 2, big.NewInt(0), uint64(1000000), big.NewInt(0), data)
		require.NoError(err)
		...
		ex2, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), contract, 3, big.NewInt(0), uint64(1000000), big.NewInt(0), data)
		require.NoError(err)
		...
		elp2 := bd.SetAction(ex2).
			...
			SetNonce(3).
			SetGasLimit(1000000).Build()
		selp2, err := action.Sign(elp2, testaddress.Addrinfo["producer"].Bech32(), testaddress.Keyinfo["producer"])
		require.NoError(err)
		...
		elp = bd.SetAction(ex3).
			...
			SetNonce(1).
			SetGasLimit(1000000).Build()
		selp3, err := action.Sign(elp, testaddress.Addrinfo["alfa"].Bech32(), testaddress.Keyinfo["alfa"])
		require.NoError(err)
...
func TestERC20(t *testing.T) {
	sct := &smartContractTest{
		...
		deploy: execCfg{
			...
			privateKey:       testaddress.Keyinfo["producer"],
			amount:           0,
		...
		executions: []execCfg{
			...
			func() execCfg {
				...
				return execCfg{
					...
					privateKey:       testaddress.Keyinfo["alfa"],
					codeHex:          hex.EncodeToString(data),
					amount:           0,
					gasLimit:         uint64(1000000),
					gasPrice:         0,
