...
func runExecution(
	...
	elp := builder.SetAction(exec).
		...
		Build()
	selp, err := action.Sign(elp, exec.Executor(), ecfg.privateKey)
	if err != nil {
		return nil, err
	}
...
func (sct *smartContractTest) deployContract(
	...
	addr, _ := address.Bech32ToAddress(contractAddress)
	copy(evmContractAddrHash[:], addr.Payload())
...
func TestProtocol_Handle(t *testing.T) {
	testEVM := func(t *testing.T) {
		...
		ws, err := sf.NewWorkingSet()
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, testaddress.Addrinfo["producer"].Bech32(), blockchain.Gen.TotalSupply)
		require.NoError(err)
		gasLimit := testutil.TestGasLimit
		ctx = protocol.WithRunActionsCtx(ctx,
			protocol.RunActionsCtx{
		...
		execution, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), action.EmptyAddress, 1, big.NewInt(0), uint64(100000), big.NewInt(0), data)
		require.NoError(err)
		...
		blk, err := bc.MintNewBlock([]action.SealedEnvelope{selp}, testaddress.Keyinfo["producer"].PubKey, testaddress.Keyinfo["producer"].PriKey, testaddress.Addrinfo["producer"].Bech32(), nil, nil, "")
		require.NoError(err)
		...
		stateDB := evm.NewStateDBAdapter(bc, ws, uint64(0), hash.ZeroHash32B, hash.ZeroHash32B)
		var evmContractAddrHash common.Address
		copy(evmContractAddrHash[:], contract.Payload())
		code := stateDB.GetCode(evmContractAddrHash)
		require.Nil(err)
		...
		exes, err := bc.GetExecutionsFromAddress(testaddress.Addrinfo["producer"].Bech32())
		require.Nil(err)
		...
		execution1, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), action.EmptyAddress, 4, big.NewInt(0), uint64(100000), big.NewInt(10), data)
		require.NoError(err)
		bd = &action.EnvelopeBuilder{}
		...
		blk, err = bc.MintNewBlock([]action.SealedEnvelope{selp}, testaddress.Keyinfo["alfa"].PubKey, testaddress.Keyinfo["alfa"].PriKey, testaddress.Addrinfo["alfa"].Bech32(), nil, nil, "")
		require.NoError(err)
		require.NoError(bc.ValidateBlock(blk, true))
		require.Nil(bc.CommitBlock(blk))
		require.Equal(1, len(blk.Receipts))
		ws, _ = sf.NewWorkingSet()
		alfaAccount, _ := account.LoadOrCreateAccount(ws, testaddress.Addrinfo["alfa"].Bech32(), blockchain.Gen.TotalSupply)
	...
	testRollDice := func(t *testing.T) {
		...
		_, err = account.LoadOrCreateAccount(ws, testaddress.Addrinfo["producer"].Bech32(), blockchain.Gen.TotalSupply)
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, testaddress.Addrinfo["alfa"].Bech32(), big.NewInt(0))
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, testaddress.Addrinfo["bravo"].Bech32(), big.NewInt(12000000))
		require.NoError(err)
		gasLimit := testutil.TestGasLimit
		ctx = protocol.WithRunActionsCtx(ctx,
			protocol.RunActionsCtx{
		...
		execution, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), action.EmptyAddress, 1, big.NewInt(0), uint64(1000000), big.NewInt(0), data)
		require.NoError(err)
		...
		execution, err = action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), contractAddr, 2, big.NewInt(500000000), uint64(120000), big.NewInt(0), data)
		require.NoError(err)
		...
		// verify balance
		balance, err = bc.Balance(testaddress.Addrinfo["alfa"].Bech32())
		require.NoError(err)
		...
		execution, err = action.NewExecution(
			testaddress.Addrinfo["bravo"].Bech32(), contractAddr, 1, big.NewInt(0), uint64(120000), big.NewInt(10), data)
		require.NoError(err)
		...
		selp, err = action.Sign(elp, testaddress.Addrinfo["bravo"].Bech32(), testaddress.Keyinfo["bravo"].PriKey)
		require.NoError(err)
		log.S().Infof("execution %+v\n", execution)
	...
	testERC20 := func(t *testing.T) {
		...
		_, err = account.LoadOrCreateAccount(ws, testaddress.Addrinfo["alfa"].Bech32(), big.NewInt(0))
		require.NoError(err)
		_, err = account.LoadOrCreateAccount(ws, testaddress.Addrinfo["bravo"].Bech32(), big.NewInt(0))
		require.NoError(err)
		gasLimit := testutil.TestGasLimit
		ctx = protocol.WithRunActionsCtx(ctx,
			protocol.RunActionsCtx{
		...
		execution, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), action.EmptyAddress, 1, big.NewInt(0), uint64(10000000), big.NewInt(0), data)
		require.NoError(err)
		...
		addr, _ := address.Bech32ToAddress(testaddress.Addrinfo["alfa"].Bech32())
		to := addr.Payload()
		...
		execution, err = action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), contract, 2, big.NewInt(0), uint64(10000000), big.NewInt(0), data)
		require.NoError(err)
		...
		addr, _ = address.Bech32ToAddress(testaddress.Addrinfo["bravo"].Bech32())
		to = addr.Payload()
		bravo.SetBytes(to)
		value = hash.ZeroHash32B
		h = value[24:]
		...
		ex2, err := action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), contract, 3, big.NewInt(0), uint64(10000000), big.NewInt(0), data)
		require.NoError(err)
		...
		elp2 := bd.SetAction(ex2).
			...
			SetNonce(3).
			SetGasLimit(10000000).Build()
		selp2, err := action.Sign(elp2, testaddress.Addrinfo["producer"].Bech32(), testaddress.Keyinfo["producer"].PriKey)
		require.NoError(err)

		blk, err = bc.MintNewBlock([]action.SealedEnvelope{selp, selp2}, testaddress.Keyinfo["producer"].PubKey, testaddress.Keyinfo["producer"].PriKey, testaddress.Addrinfo["producer"].Bech32(), nil, nil, "")
		require.NoError(err)
		require.NoError(bc.ValidateBlock(blk, true))
		require.Nil(bc.CommitBlock(blk))
		...
		ex3, err := action.NewExecution(
			testaddress.Addrinfo["alfa"].Bech32(), contract, 1, big.NewInt(0), uint64(10000000), big.NewInt(0), data)
		require.NoError(err)
		...
		elp = bd.SetAction(ex3).
			...
			SetNonce(1).
			SetGasLimit(10000000).Build()
		selp3, err := action.Sign(elp, testaddress.Addrinfo["alfa"].Bech32(), testaddress.Keyinfo["alfa"].PriKey)
		require.NoError(err)

		blk, err = bc.MintNewBlock([]action.SealedEnvelope{selp3}, testaddress.Keyinfo["alfa"].PubKey, testaddress.Keyinfo["alfa"].PriKey, testaddress.Addrinfo["alfa"].Bech32(), nil, nil, "")
		require.NoError(err)
		require.NoError(bc.ValidateBlock(blk, true))
		require.Nil(bc.CommitBlock(blk))
		...
		log.L().Warn("TestER", zap.Binary("v", data[:]))
		execution, err = action.NewExecution(
			testaddress.Addrinfo["producer"].Bech32(), contract, 4, big.NewInt(0), uint64(10000000), big.NewInt(0), data)
		require.NoError(err)
		bd = &action.EnvelopeBuilder{}
		elp = bd.SetAction(execution).
			SetDestinationAddress(contract).
...
func TestProtocol_Validate(t *testing.T) {
	...
	ex, err = action.NewExecution(
		...
		testaddress.Addrinfo["bravo"].Bech32()+"bbb",
		uint64(1),
		big.NewInt(0),
		uint64(0),
		big.NewInt(0),
...
func TestERC20(t *testing.T) {
	sct := &smartContractTest{
		prepare: map[string]*big.Int{
			testaddress.Addrinfo["producer"].Bech32(): blockchain.Gen.TotalSupply,
			testaddress.Addrinfo["alfa"].Bech32():     big.NewInt(0),
			testaddress.Addrinfo["bravo"].Bech32():    big.NewInt(0),
		},
		deploy: execCfg{
			...
			executor:         testaddress.Addrinfo["producer"].Bech32(),
		...
		executions: []execCfg{
			func() execCfg {
				data, _ := hex.DecodeString("a9059cbb")
				alfa := hash.ZeroHash32B
				to := testaddress.Addrinfo["alfa"].Payload()
				alfa.SetBytes(to)
				value := hash.ZeroHash32B
				// send 10000 token to Alfa
				h := value[24:]
			...
			func() execCfg {
				data, _ := hex.DecodeString("a9059cbb")
				bravo := hash.ZeroHash32B
				to := testaddress.Addrinfo["bravo"].Payload()
				bravo.SetBytes(to)
				value := hash.ZeroHash32B
				h := value[24:]
				binary.BigEndian.PutUint64(h, 20000)
