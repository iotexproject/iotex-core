...
func TestProtocol(t *testing.T) {
	...
	stk, err := NewProtocol(nil, genesis.Default.Staking, nil, false, genesis.Default.GreenlandBlockHeight)
	r.NotNil(stk)
	r.NoError(err)
...
func TestCreatePreStates(t *testing.T) {
	...
	defer ctrl.Finish()
	sm := testdb.NewMockStateManager(ctrl)
	p, err := NewProtocol(nil, genesis.Default.Staking, nil, false, genesis.Default.GreenlandBlockHeight)
	require.NoError(err)
