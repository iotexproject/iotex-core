...
func (r *ContractStakingStateReader) contract(contractAddr address.Address) (*stakingpb.SystemStakingContract, error) {
	var contract stakingpb.SystemStakingContract
	_, err := r.sr.State(
		&contract,
		metaNamespaceOption(),
...
func (r *ContractStakingStateReader) Bucket(contractAddr address.Address, bucketID uint64) (*Bucket, error) {
	...
	return LoadBucketFromProto(&ssb)
}
