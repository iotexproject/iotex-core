...
func (elp *envelope) Proto() *iotextypes.ActionCore {
	...
	case *ProposerRegister:
		actCore.Action = &iotextypes.ActionCore_ProposerRegister{ProposerRegister: act.Proto()}
	default:
...
func (elp *envelope) LoadProto(pbAct *iotextypes.ActionCore) error {
	...
	*elp = envelope{}
	...
	case pbAct.GetCandidateUpdate() != nil:
		...
		elp.payload = act
	case pbAct.GetProposerRegister() != nil:
		act := &ProposerRegister{}
		if err := act.LoadProto(pbAct.GetProposerRegister()); err != nil {
			return err
		}
		elp.payload = act
	default:
