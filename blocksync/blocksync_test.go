...
func TestBlockSyncerProcessBlockOutOfOrder(t *testing.T) {
	...
	chain1.Validator().AddActionValidators(account.NewProtocol(config.NewHeightUpgrade(cfg.Genesis)))
	...
	chain2.Validator().AddActionValidators(account.NewProtocol(config.NewHeightUpgrade(cfg.Genesis)))
