...
type FSM struct {
	fsm   fsm.FSM
	evtq  chan *consensusfsm.ConsensusEvent
...
func NewFSM(ctx FSMContext, clock clock.Clock) (*FSM, error) {
	cm := &FSM{
		evtq:  make(chan *consensusfsm.ConsensusEvent, ctx.EventChanSize()),
...
func (m *FSM) Start(c context.Context) error {
	m.wg.Add(1)
	go func() {
		running := true
		for running {
...
func (m *FSM) Stop(_ context.Context) error {
	close(m.close)
	m.wg.Wait()
	return nil
}
...
func (m *FSM) ProduceReceiveBlockEvent(block interface{}) {
...
func (m *FSM) ProduceReceiveProposalEndorsementEvent(vote interface{}) {
...
func (m *FSM) ProduceReceiveLockEndorsementEvent(vote interface{}) {
...
func (m *FSM) ProduceReceivePreCommitEndorsementEvent(vote interface{}) {
	m.produce(m.ctx.NewConsensusEvent(eReceivePreCommitEndorsement, vote), 0)
}

func (m *FSM) produceConsensusEvent(et fsm.EventType, delay time.Duration) {
	m.produce(m.ctx.NewConsensusEvent(et, nil), delay)
}

// produce adds an event into the queue for the consensus FSM to process
func (m *FSM) produce(evt *consensusfsm.ConsensusEvent, delay time.Duration) {
	if evt == nil {
		return
	}
...
func (m *FSM) handle(evt *consensusfsm.ConsensusEvent) error {
...
func (m *FSM) calibrate(evt fsm.Event) (fsm.State, error) {
	cEvt, ok := evt.(*consensusfsm.ConsensusEvent)
	if !ok {
...
func (m *FSM) prepare(evt fsm.Event) (fsm.State, error) {
...
func (m *FSM) processBlock(block interface{}) error {
...
func (m *FSM) onFailedToReceiveBlock(evt fsm.Event) (fsm.State, error) {
...
func (m *FSM) onReceiveProposalEndorsementInAcceptLockEndorsementState(evt fsm.Event) (fsm.State, error) {
	return m.onReceiveProposalEndorsement(evt, sAcceptLockEndorsement)
}

func (m *FSM) onReceiveProposalEndorsementInAcceptProposalEndorsementState(evt fsm.Event) (fsm.State, error) {
...
func (m *FSM) onStopReceivingProposalEndorsement(evt fsm.Event) (fsm.State, error) {
...
func (m *FSM) onStopReceivingLockEndorsement(evt fsm.Event) (fsm.State, error) {
...
func (m *FSM) onStopReceivingPreCommitEndorsement(evt fsm.Event) (fsm.State, error) {
