...
func (m *cFSM) handleInitBlockEvt(evt fsm.Event) (fsm.State, error) {
	...
	var err error
	if m.ctx.epoch.subEpochNum == 0 {
		blk, err = m.ctx.mintSecretBlock()
		if err != nil {
			return sInvalid, errors.Wrap(err, "error when minting a secret block")
		}
	} else {
...
func (m *cFSM) handleFinishEpochEvt(evt fsm.Event) (fsm.State, error) {
	if m.ctx.epoch.subEpochNum == 0 {
		dkgFinished, err := m.ctx.isDKGFinished()
		if err != nil {
			return sInvalid, errors.Wrap(err, "error when checking if dkg is finished")
		}
		if m.ctx.epoch.subEpochNum == 0 && dkgFinished {
