...
func newConsensusFSM(ctx *rollDPoSCtx) (*cFSM, error) {
	...
	b := fsm.NewBuilder().
		...
		AddTransition(
			sBlockPropose,
			eInitBlockPropose,
			cm.handleInitBlockProposeEvt,
			[]fsm.State{
				sAcceptPropose,         // move to next step
				sAcceptProposalEndorse, // locked in a proposal
			},
		).
...
func (m *cFSM) handleStartRoundEvt(_ fsm.Event) (fsm.State, error) {
	...
	if m.ctx.round.height == height {
		for _, s := range m.ctx.round.endorsementSets {
		...
		logger.Warn().Uint64("height", height).Uint32("round", round).Msg("try a new round")
	} else {
...
func (m *cFSM) handleInitBlockProposeEvt(evt fsm.Event) (fsm.State, error) {
	...
	if m.ctx.round.proposer != m.ctx.addr.RawAddress {
		...
		if lockProof := m.ctx.round.lockProof; lockProof != nil {
			hash := lockProof.BlockHash()
			log.Str("blockHash", hash.Hex()).Msg("locked in block from previous round")
			m.broadcastConsensusVote(hash, endorsement.PROPOSAL)
...
func (m *cFSM) handleProposeBlockTimeout(evt fsm.Event) (fsm.State, error) {
	...
	m.broadcastConsensusVote(hash.ZeroHash32B, endorsement.PROPOSAL)
...
func (m *cFSM) handleEndorseProposalEvt(evt fsm.Event) (fsm.State, error) {
	...
	// Gather enough proposal endorsements, lock
	m.ctx.round.lockProof = endorsementSet
	m.ctx.round.lockProof.SetRound(m.ctx.round.number)
...
func (m *cFSM) handleEndorseLockTimeout(evt fsm.Event) (fsm.State, error) {
	...
	logger.Warn().
		Uint64("height", m.ctx.round.height).
		Msg("didn't collect enough lock endorse before timeout")
...
func (m *cFSM) newProposeBlkEvt(blk *blockchain.Block) *proposeBlkEvt {
	return newProposeBlkEvt(blk, m.ctx.round.lockProof, m.ctx.round.number, m.ctx.clock)
}
