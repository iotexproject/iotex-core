...
func (ctx *rollDPoSCtx) OnConsensusReached() error {
	...
	if ctx.shouldHandleDKG() {
		for _, secretProposal := range pendingBlock.SecretProposals {
			if secretProposal.DstAddr() == ctx.encodedAddr {
	...
	if blkProto := pendingBlock.ConvertToBlockPb(); blkProto != nil {
		...
		if ctx.round.proposer == ctx.encodedAddr && ctx.chain.ChainAddress() != "" {
			putBlockToParentChain(ctx.rootChainAPI, ctx.chain.ChainAddress(), ctx.pubKey, ctx.priKey, ctx.encodedAddr,
				pendingBlock.Block)
		}
	} else {
...
func (ctx *rollDPoSCtx) validateProposeBlock(blk Block, expectedProposer string) bool {
	...
	if producer == ctx.encodedAddr && block.WorkingSet != nil {
...
func (ctx *rollDPoSCtx) mintSecretBlock() (*block.Block, error) {
	...
	confirmedNonce, err := ctx.chain.Nonce(ctx.encodedAddr)
	if err != nil {
	...
	for i, delegate := range ctx.epoch.delegates {
		secretProposal, err := action.NewSecretProposal(nonce, ctx.encodedAddr, delegate, secrets[i])
		if err != nil {
	...
	secretWitness, err := action.NewSecretWitness(nonce, ctx.encodedAddr, witness)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create the secret witness")
	}
	blk, err := ctx.chain.MintNewSecretBlock(secretProposals, secretWitness, ctx.pubKey, ctx.priKey,
		ctx.encodedAddr)
	if err != nil {
		return nil, err
	}
...
func (r *RollDPoS) HandleConsensusMsg(msg *iproto.ConsensusPb) error {
	...
	case iproto.ConsensusPb_ENDORSEMENT:
		...
		if eEvt.height() <= tipHeight && eEvt.endorse.Endorser() != r.ctx.encodedAddr {
