...
func TestRollDPoSCtx(t *testing.T) {
	...
	t.Run("case 1:panic because of chain is nil", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, nil, 0, nil, nil, dummyCandidatesByHeightFunc, "", nil, nil, 0)
		require.Error(err)
	})

	t.Run("case 2:panic because of rp is nil", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, b.EvmNetworkID(), nil, nil, dummyCandidatesByHeightFunc, "", nil, nil, 0)
		require.Error(err)
	})
	...
	t.Run("case 3:panic because of clock is nil", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, b.EvmNetworkID(), rp, nil, dummyCandidatesByHeightFunc, "", nil, nil, 0)
		require.Error(err)
	})
	...
	t.Run("case 4:panic because of fsm time bigger than block interval", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, b.EvmNetworkID(), rp, nil, dummyCandidatesByHeightFunc, "", nil, c, 0)
		require.Error(err)
	})
	...
	t.Run("case 5:panic because of nil CandidatesByHeight function", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, b.EvmNetworkID(), rp, nil, nil, "", nil, c, 0)
		require.Error(err)
	})

	t.Run("case 6:normal", func(t *testing.T) {
		bh := config.Default.Genesis.BeringBlockHeight
		rctx, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, b.EvmNetworkID(), rp, nil, dummyCandidatesByHeightFunc, "", nil, c, bh)
		require.NoError(err)
