...
func (e *proposeBlkEvt) toProtoMsg() *iproto.ProposePb {
	...
	if e.lockProof != nil {
		lockProof = e.lockProof.ToProtoBuf()
	}
...
func newProposeBlkEvtFromProtoMsg(pMsg *iproto.ProposePb, c clock.Clock) *proposeBlkEvt {
	...
	if pMsg.LockProof != nil {
		lockProof = &endorsement.Set{}
		err := lockProof.FromProtoBuf(pMsg.LockProof)
		if err != nil {
...
func newEndorseEvt(topic endorsement.ConsensusVoteTopic, blkHash hash.Hash32B, height uint64, round uint32, endorser *iotxaddress.Address, c clock.Clock) *endorseEvt {
