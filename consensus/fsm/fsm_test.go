...
func TestBackdoorEvt(t *testing.T) {
	...
	mockCtx.EXPECT().Logger().Return(logger.Logger()).AnyTimes()
	mockCtx.EXPECT().LoggerWithStats().Return(logger.Logger()).AnyTimes()
...
func TestStateTransitions(t *testing.T) {
	...
	t.Run("prepare", func(t *testing.T) {
		...
		t.Run("is-proposer", func(t *testing.T) {
			mockCtx.EXPECT().IsDelegate().Return(true).Times(2)
			mockCtx.EXPECT().IsProposer().Return(true).Times(2)
			mockCtx.EXPECT().Prepare().Return(time.Duration(0), nil).Times(2)
	...
	t.Run("onReceiveProposalEndorsement", func(t *testing.T) {
		...
		t.Run("is-locked", func(t *testing.T) {
			mockCtx.EXPECT().IsLocked().Return(true).Times(2)
			mockCtx.EXPECT().AddProposalEndorsement(gomock.Any()).Return(nil).Times(2)
			...
			t.Run("success", func(t *testing.T) {
				mockEndorsement := NewMockEndorsement(ctrl)
				mockCtx.EXPECT().NewLockEndorsement().Return(mockEndorsement, nil).Times(1)
				mockCtx.EXPECT().BroadcastEndorsement(gomock.Any()).Return().Times(1)
				state, err := cfsm.onReceiveProposalEndorsement(&ConsensusEvent{
					eventType: eReceiveProposalEndorsement,
					data:      NewMockEndorsement(ctrl),
	...
	t.Run("onReceiveLockEndorsement", func(t *testing.T) {
		...
		t.Run("ready-to-pre-commit", func(t *testing.T) {
			mockCtx.EXPECT().ReadyToPreCommit().Return(true).Times(2)
			mockCtx.EXPECT().AddLockEndorsement(gomock.Any()).Return(nil).Times(2)
