...
func TestNewHeightChange(t *testing.T) {
	...
	require.True(hu.IsPre(FbkMigration, uint64(5159880)))
	require.True(hu.IsPost(FbkMigration, uint64(5159881)))
	require.Panics(func() {
		hu.IsPost(-1, 0)
	})
	...
	require.Equal(hu.FbkMigrationBlockHeight(), uint64(5159881))
}
