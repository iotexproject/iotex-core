...
func TestNewHeightChange(t *testing.T) {
	...
	require.True(hu.IsPre(Goldengate, uint64(5553720)))
	require.True(hu.IsPost(Goldengate, uint64(5553721)))
	require.Panics(func() {
		hu.IsPost(-1, 0)
	})
	...
	require.Equal(hu.GoldengateBlockHeight(), uint64(5553721))
}
