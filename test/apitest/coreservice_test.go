...
func TestLogsInRange(t *testing.T) {
	...
	t.Run("blocks with four logs", func(t *testing.T) {
		testData := &api.FilterObjectWrapper{FromBlock: "1", ToBlock: "4"}
	...
	t.Run("empty log", func(t *testing.T) {
		testData := &api.FilterObjectWrapper{FromBlock: "1", ToBlock: "4", Address: []string{"0x8ce313ab12bf7aed8136ab36c623ff98c8eaad34"}}
		filter, err := getTopicsAddress(testData.Address, testData.Topics)
		require.NoError(err)
		from, err := strconv.ParseUint(testData.FromBlock, 10, 64)
		require.NoError(err)
	...
	t.Run("invalid start and end height", func(t *testing.T) {
		testData := &api.FilterObjectWrapper{FromBlock: "2", ToBlock: "1"}
		filter, err := getTopicsAddress(testData.Address, testData.Topics)
		require.NoError(err)
		from, err := strconv.ParseUint(testData.FromBlock, 10, 64)
		require.NoError(err)
	...
	t.Run("start block > tip height", func(t *testing.T) {
		testData := &api.FilterObjectWrapper{FromBlock: "5", ToBlock: "5"}
		filter, err := getTopicsAddress(testData.Address, testData.Topics)
		require.NoError(err)
		from, err := strconv.ParseUint(testData.FromBlock, 10, 64)
		require.NoError(err)
...
func BenchmarkLogsInRange(b *testing.B) {
	...
	testData := &api.FilterObjectWrapper{FromBlock: "0x1"}
...
func getTopicsAddress(addr []string, topics [][]string) (iotexapi.LogsFilter, error) {
	...
	for _, ethAddr := range addr {
		ioAddr, err := api.EthAddrToIoAddrWrapper(ethAddr)
		if err != nil {
	...
	for _, tp := range topics {
		var topic [][]byte
		for _, str := range tp {
			b, err := api.HexToBytesWrapper(str)
			if err != nil {
				return iotexapi.LogsFilter{}, err
			}
...
func setupTestCoreSerivce() (api.CoreService, blockchain.Blockchain, blockdao.BlockDAO, actpool.ActPool, func()) {
	cfg := newConfig()
	...
	svr, err := api.NewCoreServiceWrapper(cfg.API, bc, nil, sf, dao, indexer, bfIndexer, ap, registry, opts...)
	if err != nil {
		panic(err)
	}
