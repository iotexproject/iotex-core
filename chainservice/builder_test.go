...
func TestEstimateTipHeight(t *testing.T) {
	...
	t.Run("after dardanelles", func(t *testing.T) {
		blk, err := block.NewBuilder(block.RunnableActions{}).SetHeight(cfg.Genesis.DardanellesBlockHeight + 1).SignAndBuild(block.NewECDSAHeaderSigner(sk))
		r.NoError(err)
		r.Equal(blk.Height()+2, estimateTipHeight(&cfg, &blk, 10*time.Second))
		blk, err = block.NewBuilder(block.RunnableActions{}).SetHeight(cfg.Genesis.WakeBlockHeight - 3).SignAndBuild(block.NewECDSAHeaderSigner(sk))
		r.NoError(err)
		r.Equal(blk.Height()+2, estimateTipHeight(&cfg, &blk, 10*time.Second))
	...
	t.Run("before dardanelles", func(t *testing.T) {
		blk, err := block.NewBuilder(block.RunnableActions{}).SetHeight(cfg.Genesis.DardanellesBlockHeight - 100).SignAndBuild(block.NewECDSAHeaderSigner(sk))
		r.NoError(err)
	...
	t.Run("after wake", func(t *testing.T) {
		blk, err := block.NewBuilder(block.RunnableActions{}).SetHeight(cfg.Genesis.WakeBlockHeight).SignAndBuild(block.NewECDSAHeaderSigner(sk))
		r.NoError(err)
