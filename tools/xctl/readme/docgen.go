...
`
	...
	err = GenMarkdownTreeCustom(rootCmd, path, filePrepender, linkHandler)
	if err != nil {
		log.Fatal(err)
	}
}
...
func GenMarkdownTreeCustom(c *cobra.Command, dir string, filePrepender func(string) string,
	...
	if strings.Contains(filename, "xctl.md") {
		filename = filepath.Join(xctlPath, "README.md")
	}
...
func GenMarkdownCustom(c *cobra.Command, w io.Writer, linkHandler func(*cobra.Command, string) string) error {
	...
	if !c.DisableAutoGenTag {
		buf.WriteString("###### Auto generated by xctl on " + time.Now().Format("2-Jan-2006") + "\n")
	}
	_, err := buf.WriteTo(w)
	return err
}
