...
func main() {
	...
	if aps > 0 {
		...
		if err := testutil.WaitUntil(100*time.Millisecond, 60*time.Second, func() (bool, error) {
			receipt, err = util.GetReceiptByAction(svrs[0].APIServer(uint32(1)).GrpcServer, eHash)
